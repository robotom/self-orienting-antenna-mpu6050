...
// August 17, 2014
// Public Domain
// Expansion by TT 25-Apr-2019
//=============================================================================
/*
 * MPU6050 WIRING:
 * VIN to 5v. Some MPU6050 modules use 3.3 but this board seems to be tolerant. 
 * GND to GND.
 * SDA & SCL go to the dedicated pins on the Mega which is what I am testing on;
 * but on other boards this is usually analog 4 and 5.
 * INT should go to interrupt --> on the Mega this is D2,3,18, 19, 20, 21.
 */
 /*
  * LCD 1602 WIRING (Left = Arduino : Right = LCD1602):
  * Digital 2 to D7
  * Digital 3 to D6
  * Digital 4 to D5
  * Digital 5 to D4
  * 5v to Vdd
  * GND to Vss + RW + K (Vss, RW, K in parallel)(Vss is GND)(K is cathode)
  * Digital 9 to A (anode)
  * Digital 11 to E (enable)
  * Digital 12 to RS (register shift)
  * Digital 6 to Vo (analog output as a potentiometer that adjusts contrast 
  */
//=============================================================================

//  I've opted not to use the MPU6050 library, I was getting an error which timed it 
...
float corrected_angle; //this is necessary because the value that are mapped are not linear... they operate on a sine/cosine wave, so it'll only be correct at 0,90,etc.
...
void loop(){
  ...
  //this is the corrected angle (see top of code doc) ---- calculate the angle using trig ---- the relationship is not linear so you have to adjust for that (it's actually a trig wave)
